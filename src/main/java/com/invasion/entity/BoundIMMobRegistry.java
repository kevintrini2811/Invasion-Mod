...
public final class BoundIMMobRegistry {
    ...
    public static void bootstrap() {
        NeoForge.EVENT_BUS.addListener(BoundIMMobRegistry::onJoin);
        NeoForge.EVENT_BUS.addListener(BoundIMMobRegistry::onLeave);
    }
    ...
    private static void onJoin(EntityJoinLevelEvent event) {
        if (event.getEntity() instanceof Combatant<?> combatant) {
            update(event.getEntity(), combatant.getNexus());
        }
    }
