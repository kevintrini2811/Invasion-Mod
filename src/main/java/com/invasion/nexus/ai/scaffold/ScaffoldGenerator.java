...
public class ScaffoldGenerator {
    ...
    private Optional<List<ScaffoldNode>> findCheapestReachable(List<ScaffoldNode> nodes, DynamicPathNodeNavigator.PathSupplier pathSupplier, BlockPos nexusPos, BlockPos pos) {
        return nodes.stream()
            .map(node -> new Tuple<>(node, pathSupplier.findPathTo(nodeFactory, pos, nexusPos, 12, chunk -> {
                ScaffoldView.of(chunk).addScaffoldPosition(node.pos());
            })))
            .filter(pair -> isTerminating(pair.getB(), nexusPos))
            .sorted(Comparator.comparing(pair -> pair.getB().getEndNode().g))
            .map(Tuple::getA)
            .findFirst()
            .map(List::of);
    }
